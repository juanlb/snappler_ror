    ...
    class ScaffoldGenerator < Base
      ...
      def create_controller
        unless options.skip_controller?
          template 'controller.rb', "app/controllers/#{module_name}/#{plural_name}_controller.rb"

          template 'helper.rb', "app/helpers/#{module_name}/#{plural_name}_helper.rb"

          controller_actions.each do |action|
            if %w[index show new edit list].include?(action) # Actions with templates
              template "views/#{view_language}/#{action}.html.#{view_language}", "app/views/#{module_name}/#{plural_name}/#{action}.html.#{view_language}"
            end
          end

          if form_partial?
            template "views/#{view_language}/_form.html.#{view_language}", "app/views/#{module_name}/#{plural_name}/_form.html.#{view_language}"
          end
...
#          }
#          route "match '#{resource}/list' => '#{resource}#list'"

          namespaces = plural_name.split('/')
          resource = namespaces.pop
          route "scope :module => '#{module_name}' do
            match '#{resource}/block_action' => '#{resource}#block_action'
            match '#{resource}/list' => '#{resource}#list'
            resources :#{resource}
          end"
          
          if test_framework == :rspec
            template "tests/#{test_framework}/controller.rb", "spec/controllers/#{module_name}/#{plural_name}_controller_spec.rb"
          else
            template "tests/#{test_framework}/controller.rb", "test/functional/#{module_name}/#{plural_name}_controller_test.rb"
          end
        end
      end
      ...
      def all_actions
        %w[index show new list block_action create edit update destroy]
      end
      ...
      def render_form
        if form_partial?
          ...
          read_template("views/#{module_name}/_form.html.#{view_language}")
        end
      end
      ...
      def module_name_camel
        return module_name.camelize
      end
